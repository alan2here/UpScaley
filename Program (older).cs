...
namespace style
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var result = resolve2(orig2, style2, 15, score_col_sim_inc5, skip: 1, verbosity: 2, scale: null, passes: 1).to_bitmap();
        ...
        //   Resolve "result" from ("orig" & "style") also using ("replacement size" & "skip").
        //   replace "style" with "result"
        // yeild result
        private static img resolve2(img orig, img style,
            UInt16 replacement_size, Func<img, img, Int32, Int32, Int32, Int32, Int32, float> score,
            UInt16 skip = 1, byte verbosity = 2, float? scale = null, byte passes = 1)
        {
            if (scale <= 0f)
            ...
            if (verbosity > 1)
                Console.WriteLine("resolve2:");
            img result = new img();
            byte pass;
            for (pass = 0; pass < passes; pass++)
            {
                if (verbosity > 1)
                    Console.WriteLine("Pass " + (pass + 1) + " of " + passes + ".");
                if (scale != null)
                {
                    if (verbosity > 1)
        ...
        {
            if (verbosity > 3)
                throw new ArgumentException("3 is the maximum verbosity");
            if (score == null)
            ...
            UInt32 denominator = (UInt32)(verbosity > 2 ? 10000 : 1000);
            ...
            if (verbosity > 0)
            {
                Console.WriteLine("resolve: (img, img, replacement_size: " + replacement_size +
                    ", score: func, skip: " + skip + ", verbosity: " + verbosity + ")");
                Console.WriteLine("The load is about " + (UInt64)Math.Round(
                    (UInt64)orig_and_result_width_subtract_replacment_size *
                    (UInt64)orig_and_result_height_subtract_replacment_size *
                    (UInt64)style_width_subtract_replacment_size *
                    (UInt64)style_height_subtract_replacment_size * (UInt64)replacement_size *
                    (UInt64)replacement_size * 0.000000001f) + " billion.");
            }
            ...
            if (verbosity > 0)
                Console.WriteLine("end: (pixels: " + pixel + ")");
            return result;
        }
